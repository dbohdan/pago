...
func TestEdit(t *testing.T) {
	_, err := withPagoDir(func(dataDir string) (string, error) {
		// Add an entry with known content.
		cmd := exec.Command(commandPago, "--dir", dataDir, "add", "test-edit", "--multiline")
		cmd.Stdin = strings.NewReader("initial content")
		var stdout, stderr bytes.Buffer
		cmd.Stdout = &stdout
		cmd.Stderr = &stderr
		err := cmd.Run()
		...
		cmd = exec.Command(commandPago, "--dir", dataDir, "--socket", "", "edit", "test-edit")
		cmd.Stdin = c.Tty()
		cmd.Stdout = c.Tty()
		cmd.Stderr = c.Tty()
		...
		err = cmd.Start()
		if err != nil {
			return "", fmt.Errorf("failed to start edit command: %w", err)
		}
		...
		_, err = c.ExpectString("Save")
		if err != nil {
			return "", fmt.Errorf("failed to find initial content in editor: %w", err)
		}

		_, _ = c.Send(" edited")
		// Ctrl+D to save.
		_, _ = c.Send(string(rune(4)))

		err = cmd.Wait()
		if err != nil {
			return "", fmt.Errorf("edit command failed: %w", err)
		}

		// Verify the content has been updated.
		c2, err := expect.NewConsole()
		if err != nil {
			return "", fmt.Errorf("failed to create console for verification: %w", err)
		}
		defer c2.Close()

		var buf bytes.Buffer
		cmd2 := exec.Command(commandPago, "--dir", dataDir, "--socket", "", "show", "test-edit")
		cmd2.Stdin = c2.Tty()
		cmd2.Stdout = &buf
		cmd2.Stderr = c2.Tty()
		...
		err = cmd2.Start()
		if err != nil {
			return "", fmt.Errorf("failed to start show command for verification: %w", err)
		}

		_, err = c2.ExpectString("Enter password")
		if err != nil {
			return "", fmt.Errorf("failed to get password prompt for verification: %w", err)
		}
		_, _ = c2.SendLine(password)

		err = cmd2.Wait()
		if err != nil {
			return "", fmt.Errorf("show command for verification failed: %w", err)
		}

		content := strings.TrimSpace(buf.String())
		expected := "initial content edited"
		if content != expected {
			return "", fmt.Errorf("unexpected content: got %q, want %q", content, expected)
		}

		return "", nil
	})
	if err != nil {
		t.Errorf("Command `edit` failed: %v", err)
	}
}
